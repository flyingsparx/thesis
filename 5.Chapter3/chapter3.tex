...
Let $q$, which represents a particular question containing five Tweets, $t^q_i \forall 1 \leq i \leq 5$, be ranked in order of ascending interestingness score such that;
\[
...
In cases where $\sum\limits_{j=1}^5 \gscore{t^{q_i}_j} = 0$, $q_i$ is removed from $Q$ as none of $t^{q_i}_1,..., t^{q_i}_5$ have been ranked as more interesting than one another. 
...
\subsubsection{Is Interestingness More Indetifiable with Greater Score Separation?}
...
The results show that the average $\gdisparity{q}$ of all $q \in Q$ is roughly double in cases where a question is answered with precisely one confident Tweet than in cases where there was no confident answer made at all, demonstrating how more interesting information is easier to identify from the noise around it. 
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.8]{5.Chapter3/Media/twinterest.png} 
...
The validation test resulted in a set of just under 10,000 Tweets, authored by 936 unique users, that participants had made interestingness decisions on. Let $\testset{}$ be redefined as the set containing these Tweets, and in order to determine their predicted expected retweet counts as part of assigning them scores, two procedures were required to take place;
\begin{itemize}
